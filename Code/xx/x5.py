...
data_frame['month'] =  full_timeseries.dt.month
data_frame['day'] =    full_timeseries.dt.day
data_frame['year'] =   full_timeseries.dt.year
data_frame['hour'] =   full_timeseries.dt.hour
data_frame['minute'] = full_timeseries.dt.minute
data_frame['second'] = full_timeseries.dt.second
...
for units1 in units_first_layer:
    ...
    for units2 in units_second_layer:
        custom_optimizer = Adam(learning_rate=learning_rate_schedule, beta_1=0.9, beta_2=0.9, epsilon=1e-11)
...
# Generation features only
#features = ['total_dc_power_4_all_invs', 'total_ac_power_4_all_invs', 'total_yield_4_all_invs', 'year', 'month', 'day', 'hour', 'minute', 'second']
# Generation and Weather features
features = ['total_dc_power_4_all_invs', 'total_ac_power_4_all_invs', 'total_yield_4_all_invs',
            'irradiance',
            'module_temperature',
            'air_temperature',
...
r2_scorE = round(r2_score(predictions_best_model, y_test) * 100, 2)
print("r2 == ", r2_scorE)
color_actual = 'red'
color_predicted = 'blue'
plt.plot(y_test_flat, color=color_actual, label='Actual Total Yield', linewidth=2)
plt.plot(predictions_flat, color=color_predicted, label='Predicted Total Yield', linewidth=1)
plt.show()
